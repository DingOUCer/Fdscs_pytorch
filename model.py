...
def census_transform(img: torch.Tensor, kernel_size: int) -> torch.Tensor:
    ...
    offsets = [(u, v) for v in range(kernel_size) for u in range(kernel_size) if not u == census_center == v]  # 确定除了中心点以外的其他点的位置
    for u, v in offsets:
...
# 在RGB颜色空间下，对于每个像素点需要比较三个颜色通道的值，而在YCbCr颜色空间下，可以仅针对亮度通道（Y通道）进行视差计算。
# 由于色度通道（Cb和Cr通道）对于深度信息的影响较小，因此在立体匹配中只考虑亮度信息可以减少计算量，加快算法的速度。
...
class FDSCS(nn.Module):
    ...
    def forward(self, left, right):
        ...
        costs_signature = torch.cat([half_resolution_left, costs_signature], axis=1) # [16, 35, 128, 256]
