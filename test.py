...
parser.add_argument('--loadckpt', default='./checkpoints/kitti/fdscsdebug500/checkpoint_004999.ckpt',required=False, help='load the weights from a specific checkpoint')
...
def test():
    os.makedirs('./predictions2', exist_ok=True)
    for batch_idx, sample in enumerate(TestImgLoader):
        ...
        for disp_est, top_pad, right_pad, fn in zip(disp_est_np, top_pad_np, right_pad_np, left_filenames):
            ...
            fn = os.path.join("predictions2", fn.split('/')[-1])
...
def test_sample(sample):
    ...
    # return disp_ests[-1]
    return torch.squeeze(disp_ests, 1) #disp_ests = torch.squeeze(disp_ests, 1) #[B,H,W]
