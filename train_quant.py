...
# parser.add_argument('--logdir', default='./checkpoints/sceneflow/fdscs_sf_quant', help='the directory to save logs and checkpoints')
...
# train kitti
parser.add_argument('--lrepochs', type=str, default='3000,4000:10',help='the epochs to decay lr: the downscale rate')
parser.add_argument('--logdir', default='./checkpoints/kitti/fdscs_quant500', help='the directory to save logs and checkpoints')
parser.add_argument('--loadckpt', default='./checkpoints/sceneflow/fdscs_sf_quant/checkpoint_000073.ckpt',help='load the weights from a specific checkpoint,')
parser.add_argument('--resume', action='store_true', help='continue training the model')
parser.add_argument('--seed', type=int, default=1, metavar='S', help='random seed (default: 1)')
...
# scheduler = MultiStepLR(optimizer, milestones=[10], gamma=0.1) # learning rates
...
def train_sample(sample, compute_metrics=False):
    ...
    loss = F.smooth_l1_loss(disp_ests[mask], disp_gt[mask], reduction='mean') # 初始使用的


    # L1 = torch.abs(disp_ests - disp_gt)
    # pc = (L1 < 1.0).float()
